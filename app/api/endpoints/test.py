...
from app.ai.tests.fixtures.mock_pipelinesteps import WithoutBrowsingPipelineSteps, UnverifiableMockPipelineSteps
...
@router.post("/text-adjudication-search", response_model=AnalysisResponse)
async def analyze_text_with_adjundication_search(request: Request) -> AnalysisResponse:
    """
    test endpoint for adjudication_with_search fallback logic.

    uses UnverifiableMockPipelineSteps which:
    - mocks link expansion (no browser scraping)
    - returns hard-coded unverifiable results for normal adjudication
    - triggers the adjudication_with_search fallback with OpenAI web search

    ideal for:
    - testing the fallback mechanism when all sources are insufficient
    - integration testing of adjudication_with_search
    - verifying that OpenAI web search provides better results
    """
    start_time = time.time()
    msg_id = generate_message_id()

    logger.info(f"[{msg_id}] received /text-adjudication-search request with {len(request.content)} content item(s)")
    ...
    try:
        ...
        # step 2: get pipeline configuration
        logger.info(f"[{msg_id}] initializing gemini pipeline config (adjudication-with-search mode)")
        config = get_gemini_default_pipeline_config()

        # step 3: use UnverifiableMockPipelineSteps to trigger fallback
        pipeline_steps = UnverifiableMockPipelineSteps()
        logger.info(f"[{msg_id}] using UnverifiableMockPipelineSteps (will trigger adjudication_with_search fallback)")
        logger.info(f"[{msg_id}] UnverifiableMockPipelineSteps.adjudicate_claims will return all unverifiable")

        # step 4: run the async fact-checking pipeline
        logger.info(f"[{msg_id}] starting fact-check pipeline (adjudication will return unverifiable)")
        pipeline_start = time.time()
        fact_check_result = await run_fact_check_pipeline(
            data_sources,
            config,
