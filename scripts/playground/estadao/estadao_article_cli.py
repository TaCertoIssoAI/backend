#!/usr/bin/env python3
"""
estadao_article_cli.py — extract and display content from an Estadão article URL.

usage:
    python scripts/playground/estadao/estadao_article_cli.py

paste any estadao.com.br article link, press Enter, and the full content is shown.
...
from experiments.estadao_scraper.estadao_explorer import scrape_article
...
def main() -> None:
    print_header("Estadão Article Extractor")
    print_info("paste an estadao.com.br URL and press Enter  ·  empty line to quit\n")
    ...
    while True:
        ...
        if "estadao.com.br" not in raw:
            print_warning("URL does not look like an Estadão link — trying anyway")
